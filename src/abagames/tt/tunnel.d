...
public class Ring {
 private:
  ...
  private void drawRing(float r, float rr1, float rr2, int num) {
    ...
    for (int i = 0; i < num; i++) {
      GL.begin(GL.LINE_LOOP);
      scope auto p1 = new Vector3(sin(d) * r * rr1, cos(d) * r * rr1, 0);
      scope auto p2 = new Vector3(sin(d) * r * rr2, cos(d) * r * rr2, 0);
      scope auto p3 = new Vector3(sin(d + md) * r * rr2, cos(d + md) * r * rr2, 0);
      scope auto p4 = new Vector3(sin(d + md) * r * rr1, cos(d + md) * r * rr1, 0);
